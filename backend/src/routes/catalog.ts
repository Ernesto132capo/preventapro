...
catalogRouter.post("/products", async (req: AuthedRequest, res) => {
  ...
  await batch.commit(); invalidatePullCache("products"); res.status(201).json({ product: await serialProduct(id, product) });
});
...
catalogRouter.put("/products/:id", async (req: AuthedRequest, res) => {
  ...
  await batch.commit(); invalidatePullCache("products"); res.json({ product: await serialProduct(ref.id, { ...old, name: d.name, categoryId: d.categoryId ?? old.categoryId, baseCostCents: d.baseCostCents ?? old.baseCostCents, baseUnitName: d.baseUnitName ?? old.baseUnitName, updatedAt: ts }) });
});
...
catalogRouter.patch("/presentations/:id/stock", async (req, res) => {
  ...
  await group.docs[0].ref.update({ stock: quantityAvailable, updatedAt: nowIso() }); invalidatePullCache("products"); res.json({ ok: true });
});
catalogRouter.delete("/products/:id", async (req, res) => { await col.products.doc(req.params.id).update({ active: false, updatedAt: nowIso() }); invalidatePullCache("products"); res.json({ ok: true }); });
